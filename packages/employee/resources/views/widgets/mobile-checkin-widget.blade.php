...
>
    ...
    @else
        <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700">
            ...
            {{-- Header --}}
            <div class="bg-gray-900 px-5 py-4 dark:bg-gray-950">
                <div class="flex items-start justify-between gap-4">
                    ...
                    <div class="shrink-0 text-right">
                        <p class="text-[10px] text-gray-400">Giờ hiện tại</p>
                        <span
                            class="text-lg font-semibold tabular-nums text-white"
                            x-data="{ time: '' }"
            ...
            {{-- Status bar --}}
            <div class="flex flex-wrap items-center justify-between gap-2 border-b border-gray-100 bg-gray-50 px-5 py-2.5 dark:border-gray-700 dark:bg-gray-800/80">
                <div class="flex flex-wrap items-center gap-2">
                    @if ($isCheckedIn)
                        <span class="inline-flex items-center gap-1.5 rounded-full bg-success-100 px-2.5 py-1 text-xs font-medium text-success-700 dark:bg-success-900/50 dark:text-success-300">
                            <span class="h-1.5 w-1.5 rounded-full bg-success-500 animate-pulse"></span>
                            Check-in {{ $todayRecord->check_in_at?->format('H:i') }}
                        </span>
                    @else
                    ...
                    @if ($isCheckedOut)
                        <span class="inline-flex items-center gap-1.5 rounded-full bg-blue-100 px-2.5 py-1 text-xs font-medium text-blue-700 dark:bg-blue-900/50 dark:text-blue-300">
                            Check-out {{ $todayRecord->check_out_at?->format('H:i') }}
                        </span>
                    @endif
                </div>
                ...
                @if ($todayRecord && $todayRecord->total_hours)
                    <span class="text-xs text-gray-400 dark:text-gray-500">
                        <span class="font-semibold text-gray-600 dark:text-gray-300">{{ $todayRecord->total_hours }}h</span>
                    </span>
                @endif
            </div>
            ...
            <div class="px-5 py-4">
                ...
                <div class="space-y-2.5">
                    ...
                    @elseif (!$isCheckedOut)
                        <button
                            @if($checkoutMinimumMinutes > 0 && $checkInDiffMinutes < $checkoutMinimumMinutes)
                    ...
                    @else
                        <div class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-success-50 px-4 py-2.5 text-sm font-semibold text-success-700 dark:bg-success-900/30 dark:text-success-300">
                            <svg class="h-4 w-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                            </svg>
                            <span>{{ trans('packages.employee::employee_attendance.actions.already_checked_out') }}</span>
                        </div>
                    @endif
