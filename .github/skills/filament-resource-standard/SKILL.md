...
## Filament Action Namespace Guidelines

To prevent compilation or runtime issues with Filament Actions, you MUST follow these namespace rules based on where the Action is being registered:

1. **Page Actions (Header/Footer Actions)**:
   - Used inside resource page classes (e.g., `ListRecords`, `EditRecord`, `CreateRecord` under `Pages/`).
   - Namespace: `Filament\Actions\*`
   - Example: `use Filament\Actions\Action;` or `use Filament\Actions\CreateAction;`

2. **Table Actions (Row Actions, Header Actions, Bulk Actions)**:
   - Used inside the table schema configuration classes (under `Tables/`).
   - Namespace: `Filament\Actions\*`
   - Example: `use Filament\Actions\Action;` or `use Filament\Actions\DeleteAction;` or `use Filament\Actions\BulkActionGroup;`

3. **Form Actions (Suffix/Prefix/Component Actions)**:
   - Used inside form schema components or fields (e.g., `suffixActions()`).
   - Namespace: `Filament\Forms\Components\Actions\*`
   - Example: `use Filament\Forms\Components\Actions\Action;`
